...
namespace seal
{
    ...
    void BinaryEncoder::encode(const BigUInt &value, Plaintext &destination)
    {
        ...
        {
            if (is_bit_set_uint(value.data(), coeff_uint64_count, coeff_index))
            {
                destination[coeff_index] = 1;
            }
            coeff_index++;
    ...
    void BalancedEncoder::encode(uint64_t value, Plaintext &destination)
    {
        ...
        int encode_coeff_count = static_cast<int>(ceil(
    ...
    BigUInt BalancedEncoder::decode_biguint(const Plaintext &plain)
    {
        ...
        for (int bit_index = plain.significant_coeff_count() - 1; bit_index >= 0; bit_index--)
        {
            ...
            multiply_uint_uint(temp_result.data(), result_uint64_count, base_uint.data(), result_uint64_count, result_uint64_count, result);
    ...
    void BalancedEncoder::decode_biguint(const Plaintext &plain, BigUInt &destination)
    {
        ...
        for (int bit_index = plain.significant_coeff_count() - 1; bit_index >= 0; bit_index--)
        {
            ...
            set_uint_uint(result, result_uint64_count, temp_result.data());
            multiply_truncate_uint_uint(temp_result.data(), base_uint.data(), result_uint64_count, result);
    ...
    BinaryFractionalEncoder::BinaryFractionalEncoder(const SmallModulus &plain_modulus, const BigPoly &poly_modulus, 
        int integer_coeff_count, int fraction_coeff_count) : 
    ...
    Plaintext BinaryFractionalEncoder::encode(double value)
    {
        ...
        for (int i = 0; i < fraction_coeff_count_; i++)
        {
            ...
            {
                left_shift_uint(encoded_fract.data(), bits_per_uint64, coeff_count, encoded_fract.data());
            }
        }
        ...
        left_shift_uint(encoded_fract.data(), bits_per_uint64 * (coeff_count - 1 - fraction_coeff_count_),
            coeff_count, encoded_fract.data());
    ...
    double BinaryFractionalEncoder::decode(const Plaintext &plain)
    {
        ...
        if (plain.significant_coeff_count() >= coeff_count || !are_poly_coefficients_less_than(plain.data(),
            plain.coeff_count(), 1, encoder_.plain_modulus_.data(), encoder_.plain_modulus().uint64_count()))
        {
            throw invalid_argument("plain is not valid for encryption parameters");
        }
#endif
    ...
    BalancedFractionalEncoder::BalancedFractionalEncoder(const SmallModulus &plain_modulus, const BigPoly &poly_modulus, 
        int integer_coeff_count, int fraction_coeff_count, uint64_t base) : 
...
#ifdef SEAL_DEBUG
        if (plain.significant_coeff_count() >= coeff_count || !are_poly_coefficients_less_than(plain.data(),
            plain.coeff_count(), 1, encoder_.plain_modulus_.data(), encoder_.plain_modulus_.uint64_count()))
        {
            throw invalid_argument("plain is not valid for encryption parameters");
        }
#endif
    ...
    FractionalEncoder::FractionalEncoder(const SmallModulus &plain_modulus, const BigPoly &poly_modulus, 
        int integer_coeff_count, int fraction_coeff_count, uint64_t base)
    {
        if (base == 2)
        {
