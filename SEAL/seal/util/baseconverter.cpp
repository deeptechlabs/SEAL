...
namespace seal
{
    namespace util
    {
        BaseConverter::BaseConverter(const std::vector<SmallModulus> &coeff_base, int coeff_count, 
            int coeff_power, const SmallModulus &small_plain_mod)
        {
#ifdef SEAL_DEBUG
            ...
            */
            ...
            coeff_base_mod_count_ = static_cast<int>(coeff_base.size());
            aux_base_mod_count_ = static_cast<int>(coeff_base.size());
            ...
            for (int i = 0; i < bsk_base_mod_count_; i++)
            {
                bsk_small_ntt_table_.emplace_back();
            ...
            Pointer tmp_coeff(allocate_uint(coeff_products_uint64_count, pool_));
            ...
            Pointer tmp_aux(allocate_uint(aux_products_uint64_count, pool_));
        ...
        void BaseConverter::fastbconv_sk(const uint64_t *input, uint64_t *destination, const MemoryPoolHandle &pool) const
        {
            ...
            */
            ...
            // Fast convert B -> q
            Pointer temp_coeff_transition(allocate_uint(coeff_count_ * aux_base_mod_count_, pool));
            ...
            for (int i = 0; i < aux_base_mod_count_; i++)
            {
                ...
                for (int k = 0; k < coeff_count_; k++)
                {
                    temp_coeff_transition[i + (k * aux_base_mod_count_)] = multiply_uint_uint_mod(*input_ptr++, 
            ...
            for (int i = 0; i < coeff_base_mod_count_; i++)
            {
                ...
                for (int k = 0; k < coeff_count_; k++, temp_ptr++, destination_ptr++)
                {
                    ...
                    if (*temp_ptr > m_sk_div_2)
                    {
                        ...
                        m_alpha_sk[1] += add_uint64(m_alpha_sk[0], *destination_ptr, 0, m_alpha_sk);
                    ...
                    else
                    {
                        ...
                        multiply_uint64(coeff_base_array_elt_value - aux_products_all_mod_coeff_array_elt, 
                            *temp_ptr, m_alpha_sk);
                        m_alpha_sk[1] += add_uint64(*destination_ptr, m_alpha_sk[0], 0, m_alpha_sk);
                        *destination_ptr = barrett_reduce_128(m_alpha_sk, coeff_base_array_elt);
                    }
                }
            }
