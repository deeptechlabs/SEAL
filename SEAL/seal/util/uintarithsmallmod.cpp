...
namespace seal
{
    namespace util
    {
        ...
        void divide_uint_uint_mod_inplace(uint64_t *numerator, const SmallModulus &modulus, 
            int uint64_count, uint64_t *quotient, MemoryPool &pool)
        {
            // Handle base cases 
            if (uint64_count == 2) 
            {
            ...
            { 
                ...
                Pointer x1_alloc(allocate_uint(uint64_count - 2 , pool));
                uint64_t *x1 = x1_alloc.get();
                uint64_t x2[2];
                Pointer quot_alloc(allocate_uint(uint64_count, pool));
                uint64_t *quot = quot_alloc.get();
                Pointer rem_alloc(allocate_uint(uint64_count, pool));
                ...
                int remainder_uint64_count = get_significant_uint64_count_uint(rem, uint64_count);
