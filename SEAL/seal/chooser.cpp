...
namespace seal
{
    ...
    ChooserPoly::ChooserPoly(const ChooserPoly &copy) : 
        max_coeff_count_(0), max_abs_value_(), comp_(nullptr)
    {
        operator =(copy);
    }
    ...
    ChooserPoly ChooserEvaluator::add(const ChooserPoly &operand1, 
        const ChooserPoly &operand2)
    {
        ...
        return ChooserPoly(max(operand1.max_coeff_count_, 
            operand2.max_coeff_count_), 
            operand1.max_abs_value_ + operand2.max_abs_value_, 
            new AddComputation(*operand1.comp_, *operand2.comp_));
    }
    ...
    ChooserPoly ChooserEvaluator::sub(const ChooserPoly &operand1, 
        const ChooserPoly &operand2)
    {
        if (operand1.max_coeff_count_ <= 0 || operand1.comp_ == nullptr)
        {
        ...
        return ChooserPoly(max(operand1.max_coeff_count_, 
            operand2.max_coeff_count_), 
            operand1.max_abs_value_ + operand2.max_abs_value_, 
            new SubComputation(*operand1.comp_, *operand2.comp_));
    }
    ...
    ChooserPoly ChooserEvaluator::add_plain(const ChooserPoly &operand, 
        int plain_max_coeff_count, uint64_t plain_max_abs_value)
    {
        if (operand.max_coeff_count_ <= 0 || operand.comp_ == nullptr)
        {
        ...
        if (plain_max_abs_value == 0)
        {
            return ChooserPoly(operand.max_coeff_count_, 
                operand.max_abs_value_, 
                new AddPlainComputation(*operand.comp_, 
                    plain_max_coeff_count, 
                    plain_max_abs_value));
        }
        if (operand.max_abs_value_ == 0)
        ...
        return ChooserPoly(max(operand.max_coeff_count_, plain_max_coeff_count), 
        ...
        const ChooserPoly &plain_chooser_poly)
    {
        ...
        return sub_plain(operand, plain_chooser_poly.max_coeff_count_, 
            plain_chooser_poly.max_abs_value_);
    }
    ...
    ChooserPoly ChooserEvaluator::negate(const ChooserPoly &operand)
    {
        ...
        return ChooserPoly(operand.max_coeff_count_, 
            operand.max_abs_value_, 
            new NegateComputation(*operand.comp_));
    }
    ...
    bool ChooserEvaluator::select_parameters(
        ...
        int budget_gap, EncryptionParameters &destination)
    {
        return select_parameters(operands, budget_gap, 
            global_variables::default_noise_standard_deviation, 
            global_variables::default_coeff_modulus_128, destination);
    }
    ...
    void ChooserEncryptor::encrypt(const ChooserPoly &plain, 
        ChooserPoly &destination) const
    {
    ...
    void ChooserEncryptor::decrypt(const ChooserPoly &encrypted, 
        ChooserPoly &destination) const
    {
