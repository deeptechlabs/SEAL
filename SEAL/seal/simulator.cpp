...
namespace seal
{
        ...
        ciphertext_size_(ciphertext_size)
    {
        ...
        noise_[0] = 1;
        left_shift_uint(noise_.data(), noise_sig_bit_count, noise_.uint64_count(), noise_.data());
        decrement_uint(noise_.data(), noise_.uint64_count(), noise_.data());
    }
    ...
    Simulation SimulationEvaluator::relinearize(const Simulation &simulation, int decomposition_bit_count)
    {
        ...
        // First t
        BigUInt result_noise(simulation.parms_.plain_modulus().bit_count(), *simulation.parms_.plain_modulus().data());
    ...
    Simulation SimulationEvaluator::multiply(const Simulation &simulation1, const Simulation &simulation2)
    {
        ...
        BigUInt leading_factor = BigUInt(simulation1.parms_.plain_modulus().bit_count(), 
            *simulation1.parms_.plain_modulus().data()) * leading_sqrt_factor;
    ...
    Simulation SimulationEvaluator::add_plain(const Simulation &simulation, int plain_max_coeff_count, uint64_t plain_max_abs_value)
    {
        ...
        ConstPointer wide_plain_modulus = duplicate_uint_if_needed(simulation.parms_.plain_modulus().data(), simulation.parms_.plain_modulus().uint64_count(), coeff_uint64_count, false, pool_);

        // Compute summand
        Pointer quotient(allocate_uint(coeff_uint64_count, pool_));
        divide_uint_uint(simulation.coeff_modulus_.data(), wide_plain_modulus.get(), coeff_uint64_count, quotient.get(), result_noise.data(), pool_);
    ...
    Simulation SimulationEvaluator::get_fresh(const EncryptionParameters &parms, int plain_max_coeff_count, uint64_t plain_max_abs_value)
    {
        ...
        ConstPointer wide_plain_modulus = duplicate_uint_if_needed(parms.plain_modulus().data(), 
        ...
        multiply_uint_uint64(&plain_max_abs_value, 1, static_cast<uint64_t>(plain_max_coeff_count), 
            coeff_uint64_count, noise.data());
        divide_uint_uint(coeff_modulus.data(), wide_plain_modulus.get(), coeff_uint64_count, quotient,
            coeff_div_plain_modulus, pool_);
        multiply_truncate_uint_uint(noise.data(), coeff_div_plain_modulus, coeff_uint64_count, temp);
        ...
        // Result
        add_uint_uint(temp, temp_high, coeff_uint64_count, noise.data());
