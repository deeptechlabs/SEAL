        ...
        */
        ...
        public static void Main()
        {
            while (true)
            {
                ...
                Console.WriteLine("  6. Single-Threaded Performance Test");
                Console.WriteLine("  7. Multi-Threaded Performance Test");
                ...
                */
                ...
                switch (selection)
                {
                    ...
                    case 6:
                        ExamplePerformanceST();
                        break;

                    case 7: {
                        Console.Write("Thread count: ");
                        if (!int.TryParse(Console.ReadLine(), out int thCount) ||
                            thCount < 1)
                        {
                            Console.WriteLine("Invalid option.");
                            break;
                        }
                        ExamplePerformanceMT(thCount);
                        break;
                    }
        ...
        public static void ExamplePerformanceST()
        {
            PrintExampleBanner("Example: Performance Test (Single Thread)");

            /*
            In this example we time all the basic operations in single-threaded execution. 
            We use the following function to run the test.
            */
            void PerformanceTestST(SEALContext context)
            {
                var sw = new Stopwatch();

                PrintParameters(context);
                var polyModulus = context.PolyModulus;
                var coeffModulus = context.TotalCoeffModulus;
                var plainModulus = context.PlainModulus;
                ...
                */
                var evKeys = new EvaluationKeys();
                Console.Write($"Generating evaluation keys (dbc = {dbc}): ");
                sw.Start();
                keygen.GenerateEvaluationKeys(dbc, evKeys);
                sw.Stop();
                Console.WriteLine($"Done [{sw.ElapsedTicks * 1000000 / Stopwatch.Frequency} microseconds]");
                ...
                */
                var galKeys = new GaloisKeys();
                if (!context.Qualifiers.EnableBatching)
                {
                ...
                sw.Restart();
                keygen.GenerateGaloisKeys(dbc, galKeys);
                sw.Stop();
                Console.WriteLine($"Done [{sw.ElapsedTicks * 1000000 / Stopwatch.Frequency} microseconds]");
                ...
                var evaluator = new Evaluator(context);
                var crtbuilder = new PolyCRTBuilder(context);
                var encoder = new IntegerEncoder(plainModulus);

                /*
                These will hold the total times used by each operation.
                */
                long timeBatchSum = 0;
                long timeUnbatchSum = 0;
                long timeEncryptSum = 0;
                long timeDecryptSum = 0;
                long timeAddSum = 0;
                long timeMultiplySum = 0;
                long timeMultiplyPlainSum = 0;
                long timeSquareSum = 0;
                long timeRelinearizeSum = 0;
                long timeRotateRowsOneStepSum = 0;
                long timeRotateRowsRandomSum = 0;
                long timeRotateColumnsSum = 0;
                ...
                */
                ...
                /*
                Populate a List of values to batch.
                */
                var rd = new Random();
                var podVector = new List<UInt64>();
                for (var i = 0; i < crtbuilder.SlotCount; i++)
                {
                    podVector.Add((ulong)rd.Next() % plainModulus.Value);
                }
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    */
                    var plain = new Plaintext(context.Parms.PolyModulus.CoeffCount, 0);
                    sw.Restart();
                    crtbuilder.Compose(podVector, plain);
                    sw.Stop();
                    timeBatchSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;

                    /*
                    [Unbatching]
                    We unbatch what we just batched.
                    */
                    var podVector2 = new List<UInt64>(crtbuilder.SlotCount);
                    sw.Restart();
                    crtbuilder.Decompose(plain, podVector2);
                    sw.Stop();
                    timeUnbatchSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    for (var j = 0; j < podVector.Count; j++)
                    {
                        if (podVector[j] != podVector2[j])
                        {
                    ...
                    */
                    var encrypted = new Ciphertext(context.Parms);
                    sw.Restart();
                    encryptor.Encrypt(plain, encrypted);
                    sw.Stop();
                    timeEncryptSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;

                    /*
                    [Decryption]
                    We decrypt what we just encrypted.
                    */
                    var plain2 = new Plaintext(context.Parms.PolyModulus.CoeffCount, 0);
                    sw.Restart();
                    decryptor.Decrypt(encrypted, plain2);
                    sw.Stop();
                    timeDecryptSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    ...
                    additions with them.
                    */
                    var encrypted1 = new Ciphertext(context.Parms);
                    encryptor.Encrypt(encoder.Encode(i), encrypted1);
                    var encrypted2 = new Ciphertext(context.Parms);
                    ...
                    evaluator.Add(encrypted1, encrypted2);
                    sw.Stop();
                    timeAddSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency / 3;
                    ...
                    */
                    ...
                    evaluator.Multiply(encrypted1, encrypted2);
                    sw.Stop();
                    timeMultiplySum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    ...
                    */
                    sw.Restart();
                    evaluator.MultiplyPlain(encrypted2, plain);
                    sw.Stop();
                    timeMultiplyPlainSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    ...
                    */
                    ...
                    sw.Stop();
                    timeSquareSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    ...
                    */
                    sw.Restart();
                    evaluator.Relinearize(encrypted1, evKeys);
                    sw.Stop();
                    timeRelinearizeSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    ...
                    */
                    sw.Restart();
                    evaluator.RotateRows(encrypted, 1, galKeys);
                    evaluator.RotateRows(encrypted, -1, galKeys);
                    sw.Stop();
                    timeRotateRowsOneStepSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency / 2;
                    ...
                    */
                    var rowSize = crtbuilder.SlotCount / 2;
                    var randomRotation = rd.Next() % rowSize;
                    sw.Restart();
                    evaluator.RotateRows(encrypted, randomRotation, galKeys);
                    sw.Stop();
                    timeRotateRowsRandomSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                    ...
                    */
                    sw.Restart();
                    evaluator.RotateColumns(encrypted, galKeys);
                    sw.Stop();
                    timeRotateColumnsSum += sw.ElapsedTicks * 1000000 / Stopwatch.Frequency;
                ...
                var avgBatch = timeBatchSum / count;
                var avgUnbatch = timeUnbatchSum / count;
                var avgEncrypt = timeEncryptSum / count;
                var avgDecrypt = timeDecryptSum / count;
                var avgAdd = timeAddSum / count;
                var avgMultiply = timeMultiplySum / count;
                var avgMultiplyPlain = timeMultiplyPlainSum / count;
                var avgSquare = timeSquareSum / count;
                var avgRelinearize = timeRelinearizeSum / count;
                var avgRotateRowsOneStep = timeRotateRowsOneStepSum / count;
                var avgRotateRowsRandom = timeRotateRowsRandomSum / count;
                var avgRotateColumns = timeRotateColumnsSum / count;
            ...
            parms.PolyModulus = "1x^4096 + 1";
            parms.CoeffModulus = DefaultParams.CoeffModulus128(4096);
            parms.PlainModulus = 786433;
            PerformanceTestST(new SEALContext(parms));
            ...
            */
            //Console.WriteLine();
            //parms.PolyModulus = "1x^16384 + 1";
            //parms.CoeffModulus = DefaultParams.CoeffModulus128(16384);
            //parms.PlainModulus = 786433;
            //PerformanceTestST(new SEALContext(parms));

            //Console.WriteLine();
            //parms.PolyModulus = "1x^32768 + 1";
        ...
        public static void ExamplePerformanceMT(int thCount)
        {
            PrintExampleBanner($"Example: Performance Test ({thCount} Threads)");
            ...
            */
            ...
            var polyModulus = context.PolyModulus;
            var coeffModulus = context.CoeffModulus;
            var plainModulus = context.PlainModulus;
            ...
            /*
            For printing from several threads concurrently.
            */
            var outLock = new Object();

            /*
            We need a function similar to what we had in the single-threaded
            ...
            */
            void PerformanceTestMT(Object args)
            {
                var sw = new Stopwatch();

                /*
                Read the thread index and memory pool handle from args.
                */
                var thIndex = ((Tuple<int, MemoryPoolHandle>)args).Item1;
                var pool = ((Tuple<int, MemoryPoolHandle>)args).Item2;
                ...
                */
                ...
                {
                    Console.WriteLine();
                    Console.WriteLine($"Thread index: {thIndex}");
                ...
                */
                ...
                {
                    Console.WriteLine();
                    Console.WriteLine($"Test finished for thread {thIndex}");
                    Console.WriteLine($"Average batch: {avgBatch} microseconds");
                    Console.WriteLine($"Average unbatch: {avgUnbatch} microseconds");
                    Console.WriteLine($"Average encrypt: {avgEncrypt} microseconds");
                    Console.WriteLine($"Average decrypt: {avgDecrypt} microseconds");
            ...
            var thVector = new List<Thread>();
            for (var i = 0; i < thCount; i++)
            {
                thVector.Add(new Thread(PerformanceTestMT));
            }
            ...
            for (var i = 0; i < thCount; i++)
            {
                ...
                allocating from e.g. the global memory pool. 
                */
                thVector[i].Start(
                    new Tuple<int, MemoryPoolHandle>(i + 1, MemoryPoolHandle.New()));
                ...
                */
                //thVector[i].Start(
                //    new Tuple<int, MemoryPoolHandle>(i + 1, MemoryPoolHandle.Global()));
            }

            /*
            We are done here. Join the threads.
            */
            foreach (var th in thVector)
            {
                th.Join();
            }
