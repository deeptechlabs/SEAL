...
namespace SEALTest
{
    ...
    public:
        TEST_METHOD(CiphertextBasics)
        {
            ...
            Assert::IsTrue(ctxt_alloc.get() == ctxt.data());
        ...
        TEST_METHOD(SaveLoadCiphertext)
        {
            ...
            MemoryPoolHandle pool = MemoryPoolHandle::Global();
            Pointer ctxt_alloc(allocate_zero_uint(
                parms.poly_modulus().coeff_count() * static_cast<int>(parms.coeff_modulus().size()) * 3, pool));
            ctxt_alloc[0] = 1;
            ctxt_alloc[1] = 1;
            ctxt_alloc[2] = 0;
            ctxt_alloc[3] = 1;
            ...
            Assert::IsTrue(is_equal_uint_uint(ctxt.data(), ctxt2.data(), 
                parms.poly_modulus().coeff_count() * static_cast<int>(parms.coeff_modulus().size()) * 2));
