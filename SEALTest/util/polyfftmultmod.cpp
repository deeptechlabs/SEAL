    ...
    namespace util
    {
        ...
        public:
            TEST_METHOD(NussbaumerMultiplyPolyPolyCoeffMod)
            {
                ...
                PolyModulus polym(polymod.data(), coeff_count, 2);
            ...
            TEST_METHOD(NTTMultiplyPolyPoly)
            {
                MemoryPoolHandle pool = MemoryPoolHandle::New();
                {
                    ...
                    Modulus mod(modulus.data(), 1);

                    int coeff_count_power = 2;
                    NTTTables tables;
                    tables.generate(coeff_count_power, mod);
                    ntt_multiply_poly_poly(poly1.data(), poly2.data(), tables, result.get(), pool);
            ...
            TEST_METHOD(NTTDotProductBigPolyArrayNTTBigPolyArray)
            {
                ...
                BigPoly(testzero_arr1.coeff_count(), testzero_arr1.coeff_bit_count(), testzero_arr1.data(0)) = "Ax^3 + Bx^2";  // OK
                BigPoly(testzero_arr1.coeff_count(), testzero_arr1.coeff_bit_count(), testzero_arr1.data(1)) = "Cx^1";
                BigPoly(testzero_arr1.coeff_count(), testzero_arr1.coeff_bit_count(), testzero_arr1.data(2)) = "Dx^2 + Ex^1 + F";

                tables.generate(2, mod); 

                ntt_dot_product_bigpolyarray_nttbigpolyarray(testzero_arr1.data(0), testzero_arr2.data(0), 3, coeff_uint64_count*5, tables, result.data(), pool);
                ...
                test_arr1.set_zero();
                test_arr2.set_zero();
                BigPoly(test_arr1.coeff_count(), test_arr1.coeff_bit_count(), test_arr1.data(0)) = "6x^1 + 5";  // OK
                BigPoly(test_arr1.coeff_count(), test_arr1.coeff_bit_count(), test_arr1.data(1)) = "4x^3";
                BigPoly(test_arr1.coeff_count(), test_arr1.coeff_bit_count(), test_arr1.data(2)) = "3x^2 + 2x^1 + 1";
                BigPoly(test_arr2.coeff_count(), test_arr2.coeff_bit_count(), test_arr2.data(2)) = "1x^3 + 1x^2 + 1x^1 + 1";
                ntt_dot_product_bigpolyarray_nttbigpolyarray(test_arr1.data(0),test_arr2.data(0), 3, coeff_uint64_count*5, tables, result.data(), pool);
                ...
                for (int i = 0; i < 2; i++)
                {
                    ntt_negacyclic_harvey(arr2.data(i), tables, pool); 
                }
                ntt_dot_product_bigpolyarray_nttbigpolyarray(arr1.data(0), arr2.data(0), 2, 5, tables, result.data(), pool);
                ...
                BigPoly(scalartest1.coeff_count(), scalartest1.coeff_bit_count(), scalartest1.data(0)) = "2";
                BigPoly(scalartest2.coeff_count(), scalartest2.coeff_bit_count(), scalartest2.data(0)) = "4x^3 + 4x^2 + 4x^1 + 4";
                ntt_dot_product_bigpolyarray_nttbigpolyarray(scalartest1.data(0), scalartest2.data(0), 1, coeff_uint64_count*5, tables, result.data(), pool);
                ...
                BigPoly(scalar_zero_test1.coeff_count(), scalar_zero_test1.coeff_bit_count(), scalar_zero_test1.data(0)) = "17";
                BigPoly(scalar_zero_test2.coeff_count(), scalar_zero_test2.coeff_bit_count(), scalar_zero_test2.data(0)) = "0";
                ntt_dot_product_bigpolyarray_nttbigpolyarray(scalar_zero_test1.data(0), scalar_zero_test2.data(0), 1, coeff_uint64_count*5, tables, result.data(), pool);
                Assert::IsTrue(result.to_string() == "0");
            }
        };
    }
