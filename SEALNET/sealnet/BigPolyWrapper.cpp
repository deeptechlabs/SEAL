        ...
        namespace SEAL
        {
            ...
            void BigPoly::Save(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Write(stream, reinterpret_cast<const char*>(bigpoly_->data()), bigpoly_->coeff_count() * coeff_uint64_count * bytes_per_uint64);
            ...
            void BigPoly::Load(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    {
                        Read(stream, reinterpret_cast<char*>(bigpoly_->data()), read_coeff_count * coeff_uint64_count * bytes_per_uint64);
                    }
                    else
                    {
                        // Coefficients are different sizes, so read one at a time.
                        uint64_t *coeff_ptr = bigpoly_->data();
                    ...
                    {
                        set_zero_poly(bigpoly_->coeff_count() - read_coeff_count, coeff_uint64_count, bigpoly_->data() + read_coeff_count * coeff_uint64_count);
                    }
            ...
            int BigPoly::GetHashCode()
            {
                ...
                return ComputeArrayHashCode(bigpoly_->data(), bigpoly_->uint64_count());
            }
            ...
            BigPoly::BigPoly(int coeffCount, int coeffBitCount, std::uint64_t *value) : bigpoly_(nullptr), owned_(true)
            {
                try
                {
                    bigpoly_ = new seal::BigPoly(coeffCount, coeffBitCount, value);
                }
                catch (const exception &e)
                {
                    HandleException(&e);
