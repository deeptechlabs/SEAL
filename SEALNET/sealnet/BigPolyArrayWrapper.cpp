    ...
    namespace Research
    {
        namespace SEAL
        {
            ...
            BigPoly ^BigPolyArray::default::get(int polyIndex)
            {
                ...
                try
                {
                    return gcnew BigPoly(array_->coeff_count(), array_->coeff_bit_count(), array_->data(polyIndex));
                }
                catch (const exception &e)
                {
                    HandleException(&e);
            ...
            void BigPolyArray::Save(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Write(stream, reinterpret_cast<const char*>(array_->data(0)), array_->size() * array_->coeff_count() 
            ...
            void BigPolyArray::Load(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Read(stream, reinterpret_cast<char*>(array_->data(0)), array_->size() * array_->coeff_count() * array_->coeff_uint64_count() * bytes_per_uint64);
                }
                catch (const exception &e)
                {
                    HandleException(&e);
            ...
            int BigPolyArray::GetHashCode()
            {
                ...
                return ComputeArrayHashCode(array_->data(0), array_->uint64_count());
            }
