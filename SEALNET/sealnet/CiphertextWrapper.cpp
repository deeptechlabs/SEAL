    ...
    namespace Research
    {
        namespace SEAL
        {
            ...
            void Ciphertext::Save(Stream ^stream)
            {
                ...
                try
                {
                    Write(stream, reinterpret_cast<const char*>(&ciphertext_->hash_block()), 
                    ...
                    Write(stream, reinterpret_cast<const char*>(ciphertext_->data()),
            ...
            void Ciphertext::Load(Stream ^stream)
            {
                ...
                try
                {
                    Read(stream, reinterpret_cast<char*>(&ciphertext_->hash_block()), 
                        sizeof(seal::EncryptionParameters::hash_block_type));
                    int32_t size32 = 0;
                    ...
                    Read(stream, reinterpret_cast<char*>(ciphertext_->data()),
                        ciphertext_->size() * ciphertext_->poly_coeff_count() * ciphertext_->coeff_mod_count() * seal::util::bytes_per_uint64);
                }
                catch (const exception &e)
                {
            ...
            Tuple<UInt64, UInt64, UInt64, UInt64> ^Ciphertext::HashBlock::get()
            {
                ...
                return gcnew Tuple<UInt64, UInt64, UInt64, UInt64>(
                    ciphertext_->hash_block()[0],
                    ciphertext_->hash_block()[1],
                    ciphertext_->hash_block()[2],
                    ciphertext_->hash_block()[3]);
            }
            ...
            MemoryPoolHandle ^Ciphertext::Pool()
            {
                if (ciphertext_ == nullptr)
                {
                    throw gcnew ObjectDisposedException("Ciphertext is disposed");
                }
                return gcnew MemoryPoolHandle(ciphertext_->pool());
            }
