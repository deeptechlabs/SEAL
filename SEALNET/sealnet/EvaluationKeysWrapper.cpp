        ...
        namespace SEAL
        {
            ...
            void EvaluationKeys::Save(Stream ^stream)
            {
                ...
                try
                {
                    // Save the hash block
                    Write(stream,
                        reinterpret_cast<const char*>(&keys_->hash_block()),
            ...
            void EvaluationKeys::Load(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    // Read the hash block
                    Read(stream,
                        reinterpret_cast<char*>(&keys_->hash_block()),
                        sizeof(seal::EncryptionParameters::hash_block_type)
                    );
                    ...
                    for (int32_t i = 0; i < readSize32; i++)
                    {
                        ...
                        keys_->data()[i].reserve(keySize32);
                        for (int32_t j = 0; j < keySize32; j++)
                        {
                            key->Load(stream);
                            keys_->data()[i].emplace_back(key->GetCiphertext());
                        }
                        GC::KeepAlive(key);
                    }
                }
