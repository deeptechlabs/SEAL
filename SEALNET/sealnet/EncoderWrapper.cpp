...
namespace Microsoft
{
    namespace Research
    {
        namespace SEAL
        {
            ...
            BalancedFractionalEncoder::BalancedFractionalEncoder(SmallModulus ^plainModulus, BigPoly ^polyModulus, 
                int integerCoeffCount, int fractionCoeffCount) : fractionalEncoder_(nullptr)
            {
                if (plainModulus == nullptr)
                {
                    throw gcnew ArgumentNullException("plainModulus cannot be null");
                ...
                try
                {
                    fractionalEncoder_ = new seal::BalancedFractionalEncoder(plainModulus->GetModulus(), 
                        polyModulus->GetPolynomial(), integerCoeffCount, fractionCoeffCount);
                    GC::KeepAlive(plainModulus);
                    GC::KeepAlive(polyModulus);
                }
                catch (const exception &e)
            ...
            FractionalEncoder::FractionalEncoder(SmallModulus ^plainModulus, BigPoly ^polyModulus, 
                int integerCoeffCount, int fractionCoeffCount, UInt64 base) : fractionalEncoder_(nullptr)
            {
                if (plainModulus == nullptr)
                {
                    throw gcnew ArgumentNullException("plainModulus cannot be null");
                ...
                try
                {
                    fractionalEncoder_ = new seal::FractionalEncoder(plainModulus->GetModulus(), 
                        polyModulus->GetPolynomial(), integerCoeffCount, fractionCoeffCount, base);
                    GC::KeepAlive(plainModulus);
                    GC::KeepAlive(polyModulus);
                }
                catch (const exception &e)
