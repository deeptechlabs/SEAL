    ...
    namespace Research
    {
        namespace SEAL
        {
            ...
            int Plaintext::GetHashCode()
            {
                ...
                return ComputeArrayHashCode(plaintext_->data(), plaintext_->coeff_count());
            }
            ...
            void Plaintext::Save(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Write(stream, reinterpret_cast<const char*>(plaintext_->data()), 
            ...
            void Plaintext::Load(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Read(stream, reinterpret_cast<char*>(plaintext_->data()), 
                        plaintext_->coeff_count() * seal::util::bytes_per_uint64);
                }
                catch (const exception &e)
                {
            ...
            MemoryPoolHandle ^Plaintext::Pool()
            {
                if (plaintext_ == nullptr)
                {
                    throw gcnew ObjectDisposedException("Plaintext is disposed");
                }
                return gcnew MemoryPoolHandle(plaintext_->pool());
            }
