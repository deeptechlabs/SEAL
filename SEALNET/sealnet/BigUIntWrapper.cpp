        ...
        namespace SEAL
        {
            ...
            void BigUInt::Save(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Write(stream, reinterpret_cast<const char*>(biguint_->data()), uint64_count * bytes_per_uint64);
                }
                catch (const exception &e)
                {
                    HandleException(&e);
            ...
            void BigUInt::Load(Stream ^stream)
            {
                ...
                try
                {
                    ...
                    Read(stream, reinterpret_cast<char*>(biguint_->data()), read_uint64_count * bytes_per_uint64);

                    // Zero any extra space.
                    int uint64_count = divide_round_up(biguint_->bit_count(), bits_per_uint64);
                    if (uint64_count > read_uint64_count)
                    {
                        set_zero_uint(uint64_count - read_uint64_count, biguint_->data() + read_uint64_count);
                    }
                }
                catch (const exception &e)
                {
            ...
            int BigUInt::GetHashCode()
            {
                ...
                return ComputeArrayHashCode(biguint_->data(), biguint_->uint64_count());
            }
